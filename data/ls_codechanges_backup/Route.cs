...
namespace ServiceSiteScheduling.Routing
{
    class Route : IEquatable<Route>
    {
        ...
        {
            // Time result = switchcount[index] * Settings.SwitchCrossingTime + reversals * train.ReversalDuration;
            // int count = trackcount[index];
            // if (count % 2 != 0) {
            //     count += 1;
            // }
            // result += (reversals + count) * Settings.TrackCrossingTime;
            this.Duration = this.TotalSwitches * Settings.SwitchCrossingTime
                + this.TotalReversals * this.Train.ReversalDuration;
            // Console.WriteLine(this.Duration);
            Time count = this.Tracks.Length;
            count = (count-1)*2;
            this.Duration = (count) * Settings.TrackCrossingTime;
            // if (this.Duration == 20) {
            // Console.WriteLine(count);
            // Console.WriteLine(this.Tracks.Length);
            // Console.WriteLine(this.Duration);
            // this.Duration =
