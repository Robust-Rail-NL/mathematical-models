...
def setup(location, scenario):
  ...
  return nodes, edges, conflict_edges, agents, start_nodes, arrival_time, departures, start_time, end_time, train_types, time_window, lambda_values, mu_values, r, cost

def create_model(agents, a, nodes, edges, conflict_edges, start_nodes, arrival_time, departures, train_types, time_window, lambda_values, mu_values, r, cost):
  model = ConcreteModel()
...
def solve_agent(k, a, m, nodes, edges, conflict_edges, time_window, lambda_values, mu_values, cost):
...
def Lagrangian(nodes, edges, conflict_edges, agents, start_nodes, arrival_time, departures, start_time, end_time, train_types, time_window, lambda_values, mu_values, r, cost):
  n_iter = 250
  objectives = []
  models = {}
  obj = 0
  n_agents = len(agents)
  for a in agents:
    models[a] = create_model(agents, a, nodes, edges, conflict_edges, start_nodes, arrival_time, departures, train_types, time_window, lambda_values, mu_values, r, cost)
  ...
  for k in range(n_iter):
    ...
    for a in agents:
      x_values[a], p_values[a], y_values[a], objective_values[a]= solve_agent(k,a, models[a], nodes, edges, conflict_edges, time_window, lambda_values, mu_values, cost)
    #region print
    ...
    for t in time_window:
      ...
      for g in range(1, len(conflict_edges)+1):
        ...
        if penalty > 0:
          # print(f"x_penalty[{g},{t}] = {x_penalty[g,t]}, penalty = {penalty}")
...
if __name__ == "__main__":
  ...
  nodes, edges, conflict_edges, agents, start_nodes, arrival_time, departures, start_time, end_time, train_types, time_window, lambda_values, mu_values, r, cost = setup(location, scenario)
  k, time = Lagrangian(nodes, edges, conflict_edges, agents, start_nodes, arrival_time, departures, start_time, end_time, train_types, time_window, lambda_values, mu_values, r, cost)
